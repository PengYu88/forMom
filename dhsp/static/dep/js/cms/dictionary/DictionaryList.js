tsc.DictionaryList = {
	...
	doQuery: function(page){
		...
		tsc.DictionaryList.queryDictType();
	},

	//查询分类信息
	queryDictType : function (){
		$.getJSON("/content/dictionary/queryDictionaryType", function(rs) {
			// 循环遍历数据
		...
		sweetAlert({
			...
			function(isConfirm){
				if (isConfirm) {
                    ...
                    $.ajax({
                        url: "/content/dictionary/deleteDictionary",
                        data: {
                        	"dictId":dictId
                        },
                        dataType:"json",
